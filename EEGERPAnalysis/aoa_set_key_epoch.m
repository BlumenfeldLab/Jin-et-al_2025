clear all
clc
%% Iterate over sessions, finding which epochs (amongst all; aware, unaware, or other) are designated as such
location = 's'
...
for session = 1:length(text);
    ...
    if isfile([fileLocation sessionDate '_good_epoch_designations_quizzes_then_choose.mat']) && isfile([fileLocation sessionDate '_good_epoch_designations_middles.mat']);
        disp('Epochs already redone.')
        continue
    end
    disp(['Analyzing ' text{1,session} ])
    load([fileLocation sessionDate '_good_epoch_designations.mat'])
    load([fileLocation sessionDate '_goodConfirmEpochs.mat'])
    load([fileLocation sessionDate '_good_confirm_key_epochs.mat'])
    load([fileLocation sessionDate '_sliders_and_percentiles.mat'])
    load([fileLocation sessionDate '_bad_epochs.mat'])
    ...
    if ~isfile([fileLocation sessionDate '_good_epoch_designations_quizzes_then_choose.mat'])
        eval(['save ' fileLocation sessionDate '_good_epoch_designations_quizzes_then_choose.mat good_epoch_designations -v7.3'])
    end
    % Designate other outcomes besides aware and unaware
    ...
    if ~isfile([fileLocation sessionDate '_good_epoch_designations_middles.mat'])
        eval(['save ' fileLocation sessionDate '_good_epoch_designations_middles.mat good_epoch_designations -v7.3'])
    end
