%% Creates group mean data for late runs (4-6)
...
for session = 1:length(allSubjectNames)
    ...
    load([sessionDates{session} '_mean_erps_split_designations_runs.mat'])
    mean_aware_erp = mean_aware_erp_late_runs;
    mean_unaware_erp = mean_unaware_erp_late_runs;
    awareTotals = awareTotals_late_runs;
    unawareTotals = unawareTotals_late_runs;
...
creationFunction = 'aoa_hpc_plot_mean_erp_split_designations_runs_late.m'
save('/vast/palmer/pi/blumenfeld/dsj8/AoA_Study/aoa_group_erp_means_split_designations_runs_late.mat','avg_erp_all_channels_aware_subjects','avg_erp_all_channels_unaware_subjects','allSubjectMeansAware','allSubjectMeansUnaware','creationFunction')
