...
timeMin = 20;
electrodeMin = 20;
creationFunction = 'analyze_average_cluster_size_clicks.m';
creationDate = string(datetime('now'))
for band = 1:length(bands)
    currentBand = bands{band}
    load([currentBand '_permutation_stat_cluster_sumt_rand_005_5000_perm_' data_type '_clicks.mat'])
...
%     figure
%     hold on
%     eval(['imagesc(topoplot_data_' currentBand ')'])
%     ylim([0 257])
%     xlim([0 4000])
...
%     figure
%     hold on
%     histogram(averageClusterSize)
%     title(['Average Cluster Size by Electrode, ' num2str(currentBand)])
%     set(gca,'FontSize',24)
%     xlabel('Length (ms)')
...
%     figure
%     hold on
%     histogram(allClusterSizes)
%     title(['All Spatiotemporal Cluster Sizes, ' num2str(currentBand)])
%     xlabel('Length (ms)')
%     ylabel('Number of Electrodes')
%     drawnow
%     
...
%     timeMin = prctile(all_cluster_timepoints_counts,75);
%     electrodeMin = prctile(all_cluster_electrode_counts,75);
...
save(['topoplot_data_filtered' num2str(timeMin) 'ms_' num2str(electrodeMin) 'electrodes_' data_type '_clicks.mat'],'topoplot_data_filtered_voltage','creationFunction','creationDate');
