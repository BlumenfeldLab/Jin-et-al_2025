%% This script looks over all the clusters following permutation analysis
...
addpath('D:\')
addpath('D:\Spatiotemporal_Filter\Cluster_Permutation_Voltage_v2')
cd('D:\Spatiotemporal_Filter\Cluster_Permutation_Voltage_v2')
data_condition = 'Aware_minus_Unaware' % Conditions are performed separately
types = {'voltage'}; % No other data types are used
timeMin = 20;
electrodeMin = 20;
creationDate = string(datetime('now'))
suffix = 'all_counterbalance'; 
for type = 1:length(types)
    currentData = types{type}
    load([currentData '_permutation_stat_cluster_sumt_rand_005_5000_perm_' data_condition '_' suffix '.mat'])
    eval(['pval_' currentData ' = pval;'])
    creationFunction = 'analyze_average_cluster_size_voltage.m';
    ...
    for electrodeChannel = 1:257
        disp(['Evaluating ' currentData ', channel ' num2str(electrodeChannel)])
    ...
    eval(['allClusterSizes_' currentData ' = allClusterSizes;'])
    eval(['averageClusterSize_' currentData ' = averageClusterSize;'])
    topoplot_data(topoplot_data < 0) = -1;
    topoplot_data(topoplot_data > 0) = 1;
    eval(['topoplot_data_' currentData ' = topoplot_data;'])
    eval(['clust_info_' currentData ' = clust_info;'])
    topoplot_data_scaled = topoplot_data;
    % Analyze positive clusters and negative clusters separately
    ...
    eval( ['topoplot_data_scaled_' currentData ' = topoplot_data_scaled;'])
    figure
    hold on
    eval(['imagesc(topoplot_data_scaled_' currentData ')'])
    ylim([0 257])
    xlim([0 4000])
    ...
    title(['Clusters and Electrodes, ' num2str(currentData)])
    drawnow
    ...
    figure;
    title(['Cluster Details, ' currentData])
    subplot(1,2,1)
    ...
    eval( ['topoplot_data_filtered_' currentData ' = topoplot_data_filtered;'])

    figure
    hold on
    eval(['imagesc(topoplot_data_filtered_' currentData ')'])
    ylim([0 257])
    xlim([0 4000])
    set(gcf, 'WindowState', 'maximized')
    xt = get(gca, 'XTick');
    ...
    ylabel('Electrode Channel')
    xlabel('Time from Confirm')
    title(['Clusters and Electrodes, min ' num2str(timeMin)  ' (ms), ' num2str(electrodeMin) ' electrodes, ' num2str(currentData)])
    drawnow
    ...
    for channel = 1:257
        ...
        if length(end_indices_pos) < length(start_indices_pos)
            end_indices_pos = [end_indices_pos 4000];
            start_indices_neg = [start_indices_neg 4000];
        end

        if length(end_indices_neg) < length(start_indices_neg)
            
            end_indices_neg = [end_indices_neg 4000];
            start_indices_pos = [start_indices_pos 4000];
        end
        ...
        topoplot_data_filtered_voltage(channel,:) = result_vector;
    end
    ...
    ylabel('Electrode Channel')
    xlabel('Time from Confirm')
    title(['Clusters and Electrodes, min ' num2str(timeMin)  ' (ms), ' num2str(electrodeMin) ' electrodes, ' num2str(currentData) ' min 20 ms'])
    drawnow
    
end 

save(['topoplot_data_filtered' num2str(timeMin) 'ms_' num2str(electrodeMin) 'electrodes_' data_condition '_' suffix '.mat'],'topoplot_data_filtered_voltage','creationFunction','creationDate');
