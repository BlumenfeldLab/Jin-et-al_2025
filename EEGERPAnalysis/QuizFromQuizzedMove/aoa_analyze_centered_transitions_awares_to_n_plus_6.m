% Analyzes whether a quiz from the current move was aware or unaware.
clear all
...
creationFunction = 'aoa_analyze_centered_transitions_awares_to_n_plus_6.m';
...
for session = 1:length(text)
    ...
    if isfile([sessionName '_centered_transitions_awares.mat'])
        ...
        for quiz = 1:6
            ...
            if sliderSuccessesAllPercentilesTiming(quiz,3) < 25 && sliderSuccessesAllPercentilesTiming(quiz,4) == 0
                for remainingquiz = 1:13-min(find(~isnan(currentQuiz)))
                    if sliderSuccessesAllPercentilesTiming(remainingquiz,3) > 75 && sliderSuccessesAllPercentilesTiming(remainingquiz,4) == 1
        ...
        for quiz = 7:length(sliderSuccessesAllPercentilesTiming)-6
            ...
            currentQuiz(7) = 0;
            if sliderSuccessesAllPercentilesTiming(quiz,3) < 25 && sliderSuccessesAllPercentilesTiming(quiz,4) == 0
                if sliderSuccessesAllPercentilesTiming(quiz-6,3) > 75 && sliderSuccessesAllPercentilesTiming(quiz-6,4) == 1
                    currentQuiz(1) = 1;
                end
                if sliderSuccessesAllPercentilesTiming(quiz-5,3) > 75 && sliderSuccessesAllPercentilesTiming(quiz-5,4) == 1
                    currentQuiz(2) = 1;
                end
                if sliderSuccessesAllPercentilesTiming(quiz-4,3) > 75 && sliderSuccessesAllPercentilesTiming(quiz-4,4) == 1
                    currentQuiz(3) = 1;
                end
                if sliderSuccessesAllPercentilesTiming(quiz-3,3) > 75 && sliderSuccessesAllPercentilesTiming(quiz-3,4) == 1
                    currentQuiz(4) = 1;
                end
                if sliderSuccessesAllPercentilesTiming(quiz-2,3) > 75 && sliderSuccessesAllPercentilesTiming(quiz-2,4) == 1
                    currentQuiz(5) = 1;
                end
                if sliderSuccessesAllPercentilesTiming(quiz-1,3) > 75 && sliderSuccessesAllPercentilesTiming(quiz-1,4) == 1
                    currentQuiz(6) = 1;
                end
                if sliderSuccessesAllPercentilesTiming(quiz+1,3) > 75 && sliderSuccessesAllPercentilesTiming(quiz+1,4) == 1
                    currentQuiz(8) = 1;
        ...
        for quiz = length(sliderSuccessesAllPercentilesTiming)-5:length(sliderSuccessesAllPercentilesTiming)
            ...
            if sliderSuccessesAllPercentilesTiming(quiz,3) < 25 && sliderSuccessesAllPercentilesTiming(quiz,4) == 0
                for remainingquiz = 1:length(remaining_indices)
                    if sliderSuccessesAllPercentilesTiming(remaining_indices(remainingquiz),3) > 75 && sliderSuccessesAllPercentilesTiming(remaining_indices(remainingquiz),4) == 1
        ...
        save([sessionName '_centered_transitions_awares.mat'],'currentsession_aware_sequences','currentsession_unaware_sequences','creationFunction','creationDate')
    ...
    load([sessionName '_centered_transitions_awares.mat'])
...
hold on
title(['Awareness Rates, N = ' num2str(length(all_subject_mean_aware))])
