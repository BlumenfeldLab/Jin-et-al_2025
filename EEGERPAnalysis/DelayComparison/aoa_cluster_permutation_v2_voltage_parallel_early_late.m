...
function output_end = aoa_cluster_permutation_v2_voltage_parallel_early_late(data_type,window,suffix1,suffix2,saveSuffix)
    %% EEG Electrode Based Permutation Analysis for testing the same condition, albeit with an early late split
    
    % Inputs: 
    % data_type: A string of the testing condition (e.g. Aware_Early_minus_Aware_Late)
    % suffix1 and suffix2: May be either for delays "raw" or for runs "runs"
    ...
    load('/vast/palmer/pi/blumenfeld/dsj8/AoA_Study/aoa_EEGlab_blank.mat');

    %Photogrammetry directory
    Photo_dir = '/vast/palmer/pi/blumenfeld/dsj8/AoA_Study/eeglab14_0_0b/sample_locs/GSN-HydroCel-257.sfp';

    %Data directory
    data_dir = '/vast/palmer/pi/blumenfeld/dsj8/AoA_Study/';

    %Save directory
    save_dir = fullfile('/vast/palmer/pi/blumenfeld/dsj8/AoA_Study/Cluster_Permutation_Voltage/');

    %Neighborhood matrix directory
    hood_dir = '/vast/palmer/pi/blumenfeld/dsj8/AoA_Study/';
    ...
    if ~strcmp(suffix1, '')
        load(['aoa_group_erp_means_' suffix1 '.mat']) 
    elseif strcmp(suffix1,'')
        load(['aoa_group_erp_means.mat']) 
    end
    avg_erp_all_channels_aware_subjects_1 = avg_erp_all_channels_aware_subjects;
    avg_erp_all_channels_unaware_subjects_1 = avg_erp_all_channels_unaware_subjects;
    
    if ~strcmp(suffix2, '')
        load(['aoa_group_erp_means_' suffix2 '.mat']) 
    elseif strcmp(suffix2,'')
        load(['aoa_group_erp_means.mat']) 
    end
    avg_erp_all_channels_aware_subjects_2 = avg_erp_all_channels_aware_subjects;
    avg_erp_all_channels_unaware_subjects_2 = avg_erp_all_channels_unaware_subjects;
    ...
    %Subtraction
    if isequal(data_type, 'Aware_Early_Late')

    
        %Subtract main data from subtract data
        group_pc_data = avg_erp_all_channels_aware_subjects_1 - avg_erp_all_channels_aware_subjects_2;
    ...
    elseif isequal(data_type, 'Unaware_Early_Late')

        %Subtract main data from subtract data
        group_pc_data = avg_erp_all_channels_unaware_subjects_1 - avg_erp_all_channels_unaware_subjects_2;
    ...
    for clust = 1:length(pos_clust)
        ...
        for time = 1:size(empty_matrix,2)
            ...
            [sig_grid_electrodes, col] = find(clust_info.pos_clust_ids(:,time) == pos_clust(clust));

            %Constant
            if isequal(plotting_method, 'c')

                %Give post clusters constant value - find the voxel all volume idx
                empty_matrix(sig_grid_electrodes,time) = 0.7;

            %T-value
            elseif isequal(plotting_method, 't')

                %T-value
                empty_matrix(sig_grid_electrodes,time) = t_orig(sig_grid_electrodes,time);
    ...
    for clust = 1:length(neg_clust)
        ...
        for time = 1:size(empty_matrix,2)
            ...
            %Find postive significant cluster number/idx in voxel x time matrix
            [sig_grid_electrodes, col] = find(clust_info.neg_clust_ids(:,time) == neg_clust(clust));
    ...
    creationFunction = 'aoa_cluster_permutation_v2_voltage_parallel_early_late.m';
    if strcmp(suffix1, '')
    ...
    elseif ~strcmp(suffix1,'')
        save(['voltage_permutation_stat_cluster_sumt_rand_005_',num2str(num_permutation),'_perm_' data_type '_' time_window '_' saveSuffix '.mat'],'pval','t_orig','clust_info','seed_state','est_alpha','mn_clust_mass' ...
        ,'group_pc_baselined_data','topoplot_data','creationFunction');
    end
