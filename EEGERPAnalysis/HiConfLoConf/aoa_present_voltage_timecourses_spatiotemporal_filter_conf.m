%% This script presents voltage timecourses for high-confidence vs low-confidence actions
saveFigures = true;
...
load([dataRoot '\topoplot_data_filtered20ms_20electrodes_HiConf_minus_LoConf_all_' suffix '.mat'])
...
for channel = 1:length(electrodeSubset)
    ...
    mean_voltage_hiConf_trace = mean(avg_erp_all_channels_hiConf_subjects(electrodeChannel,:,:),3);
    mean_voltage_hiConf_sem = std(squeeze(avg_erp_all_channels_hiConf_subjects(electrodeChannel,:,:))')/sqrt(size(avg_erp_all_channels_hiConf_subjects,3));
    
    mean_voltage_loConf_trace = mean(avg_erp_all_channels_loConf_subjects(electrodeChannel,:,:),3);
    mean_voltage_loConf_sem = std(squeeze(avg_erp_all_channels_loConf_subjects(electrodeChannel,:,:))')/sqrt(size(avg_erp_all_channels_loConf_subjects,3));
    
    mean_voltage_hiConf_high = mean_voltage_hiConf_trace + mean_voltage_hiConf_sem;
    mean_voltage_hiConf_low = mean_voltage_hiConf_trace - mean_voltage_hiConf_sem;
    
    mean_voltage_loConf_high = mean_voltage_loConf_trace + mean_voltage_loConf_sem;
    mean_voltage_loConf_low = mean_voltage_loConf_trace - mean_voltage_loConf_sem;
    ...
    lowpass_hiConf_high = filtfilt(lpFilt,mean_voltage_hiConf_trace+mean_voltage_hiConf_sem);
    lowpass_hiConf_low = filtfilt(lpFilt,mean_voltage_hiConf_trace-mean_voltage_hiConf_sem);
    lowpass_loConf_high = filtfilt(lpFilt,mean_voltage_loConf_trace+mean_voltage_loConf_sem);
    lowpass_loConf_low = filtfilt(lpFilt,mean_voltage_loConf_trace-mean_voltage_loConf_sem);
    ...
    if saveFigures
        ...
        ylabel(['Voltage (uV)'])
        patch([-1999:2000 fliplr(-1999:2000)], [lowpass_hiConf_low fliplr(lowpass_hiConf_high)], [0 0 1],'FaceVertexAlphaData',0.1,'FaceAlpha','flat','LineStyle','none')
        patch([-1999:2000 fliplr(-1999:2000)], [lowpass_loConf_low fliplr(lowpass_loConf_high)], [1 0.7 0],'FaceVertexAlphaData',0.1,'FaceAlpha','flat','LineStyle','none')
        
        erp_legend = plot(-1999:2000,filtfilt(lpFilt,mean_voltage_hiConf_trace),'Color',[0 0 1 0.7],'LineWidth',5);
        erp_legend = [erp_legend plot(-1999:2000,filtfilt(lpFilt,mean_voltage_loConf_trace),'Color',[1 0.7 0 0.7],'LineWidth',5)];
        ...
        xlim([-1000 1000])
        set(gca,'FontSize',24)
        hLegend = legend(erp_legend,{'HiConf','LoConf'});
