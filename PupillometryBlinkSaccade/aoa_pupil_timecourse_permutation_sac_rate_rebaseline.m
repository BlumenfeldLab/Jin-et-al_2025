%% Saccade Permutation Analysis - ROI Timecourse
...
if strcmp(run_location,'l')
    data_dir = 'Y:\HNCT_AoA_Study\AoA_Subjects\';
    addpath(genpath(data_dir));    
    %Save directory
    save_dir = 'Y:\HNCT_AoA_Study\AoA_Subjects\';
...
%% Load group saccade data
cd(data_dir)
eye = 'left'
load([data_dir '/aoa_sac_rate_group_data.mat'])
binsize = 500;


if strcmp(eye,'left')

    all_avg_sac_rate_aware_temp = all_avg_sac_rate_aware_left(:,6001:10000);
    all_avg_sac_rate_unaware_temp = all_avg_sac_rate_unaware_left(:,6001:10000);
    all_avg_sac_rate_aware = [];
    all_avg_sac_rate_unaware = [];
elseif strcmp(eye,'right')
    all_avg_sac_rate_aware_temp = all_avg_sac_rate_aware_right(:,6001:10000);
    all_avg_sac_rate_unaware_temp = all_avg_sac_rate_unaware_right(:,6001:10000);
    all_avg_sac_rate_aware = [];
    all_avg_sac_rate_unaware = [];
end
...
for subject = 1:size(all_avg_sac_rate_aware_temp,1)
    currentEpochAware = [all_avg_sac_rate_aware_temp(subject,:); all_avg_sac_rate_aware_temp(subject,:)];
    all_avg_sac_rate_aware = cat(3,all_avg_sac_rate_aware,currentEpochAware);

    currentEpochUnaware = [all_avg_sac_rate_unaware_temp(subject,:); all_avg_sac_rate_unaware_temp(subject,:)];
    all_avg_sac_rate_unaware = cat(3,all_avg_sac_rate_unaware,currentEpochUnaware);
end
...
group_pc_data = all_avg_sac_rate_aware - all_avg_sac_rate_unaware;
...
%Calculate baseline values again, now calculating
%the baseline of the 
group_pc_baseline = squeeze(nanmean(group_aware_minus_unaware_baselined_data(:,baseline_window,:),2));

%Convert baseline 
group_aware_minus_unaware_baseline = permute(repmat(group_pc_baseline,[1,1,4000]),[1,3,2]);

%Mean saccade over subjects
aware_minus_unaware_mean_saccade = nanmean(group_aware_minus_unaware_baselined_data,3);

%aware

%Calculate baseline values
group_pc_baseline = squeeze(nanmean(all_avg_sac_rate_aware(:,baseline_window,:),2));

%Convert baseline
group_aware_baseline = permute(repmat(group_pc_baseline,[1,1,4000]),[1,3,2]);

%Subtract baseline from main data
group_aware_baselined_data = all_avg_sac_rate_aware - group_aware_baseline;

%Calculate baseline values
group_pc_baseline = squeeze(nanmean(group_aware_baselined_data(:,baseline_window,:),2));
...
%Mean saccade over subjects
aware_mean_saccade = nanmean(group_aware_baselined_data,3);

%unaware
    
%Calculate baseline values
group_pc_baseline = squeeze(nanmean(all_avg_sac_rate_unaware(:,baseline_window,:),2));

%Convert baseline 
group_unaware_baseline = permute(repmat(group_pc_baseline,[1,1,4000]),[1,3,2]);

%Subtract baseline from main data
group_unaware_baselined_data = all_avg_sac_rate_unaware - group_unaware_baseline;

%Calculate baseline values 
group_pc_baseline = squeeze(nanmean(group_unaware_baselined_data(:,baseline_window,:),2));

%Convert baseline 
group_unaware_baseline = permute(repmat(group_pc_baseline,[1,1,4000]),[1,3,2]);

%Mean saccade over subjects
unaware_mean_saccade = nanmean(group_unaware_baselined_data,3);
...
for chan = 1:length(eye_list)
    ...
    %Save output
    cd(save_dir)
    save(['timecourse_cluster_aware_vs_unaware_eye_',eye,'_',current_eye_name,'_',num2str(num_permutations),'perm_sac_rate.mat'],...
    ...
    %Save output
    cd(save_dir)
    save(['timecourse_cluster_aware_minus_unaware_eye_',eye,'_',current_eye_name,'_',num2str(num_permutations),'perm_sac_rate.mat'],...
        'clusters','pval','t_sums','permutation_distribution','sig_clust','sig_time_pts');
    ...
    %Save output
    cd(save_dir)
    save(['timecourse_cluster_aware_eye_',eye,'_',current_eye_name,'_',num2str(num_permutations),'perm_sac_rate.mat'],...
        'clusters','pval','t_sums','permutation_distribution','sig_clust','sig_time_pts');
    ...
    %Save output
    cd(save_dir)
    save(['timecourse_cluster_unaware_eye_',eye,'_',current_eye_name,'_',num2str(num_permutations),'perm_sac_rate.mat'],...
        'clusters','pval','t_sums','permutation_distribution','sig_clust','sig_time_pts');
    toc
