%% Pupil Run Z-Score Permutation Analysis - ROI Timecourse
...
elseif strcmp(run_location,'s')
    data_dir = '/mnt/Data27/HNCT_AoA_Study/HPC_Analyses/';
    addpath(genpath(data_dir));    
    %Save directory
    save_dir = '/mnt/Data27/HNCT_AoA_Study/HPC_Analyses/';
end
...
cd(data_dir)
load([data_dir '/aoa_eye_group_data_v4.mat'])
...
if strcmp(eye,'left')
    ...
    all_avg_pupil_aware_temp = all_avg_pupil_aware_left(:,2001:6000);
    all_avg_pupil_unaware_temp = all_avg_pupil_unaware_left(:,2001:6000);
    all_avg_pupil_aware = [];
    all_avg_pupil_unaware = [];
elseif strcmp(eye,'right')
    all_avg_pupil_aware_temp = all_avg_pupil_aware_right(:,2001:6000);
    all_avg_pupil_unaware_temp = all_avg_pupil_unaware_right(:,2001:6000);
    all_avg_pupil_aware = [];
    all_avg_pupil_unaware = [];
end
...
aware_minus_unaware_mean_voltage = nanmean(group_aware_minus_unaware_baselined_data,3);
...
%Mean voltage over subjects
aware_mean_voltage = nanmean(group_aware_baselined_data,3);
...
%Reconfigure baselines
unaware_baseline_baseline = mean(group_unaware_baseline(baseline_window),2)
...
for chan = 1:length(eye_list)
    ...
    %Save output
    cd(save_dir)
    save(['timecourse_cluster_aware_vs_unaware_eye_',eye,'_',current_eye_name,'_',num2str(num_permutations),'perm_run_baseline.mat'],...
    ...
    %Save output
    cd(save_dir)
    save(['timecourse_cluster_aware_minus_unaware_eye_',eye,'_',current_eye_name,'_',num2str(num_permutations),'perm_run_baseline.mat'],...
        'clusters','pval','t_sums','permutation_distribution','sig_clust','sig_time_pts');
    ...
    %Save output
    cd(save_dir)
    save(['timecourse_cluster_aware_eye_',eye,'_',current_eye_name,'_',num2str(num_permutations),'perm_run_baseline.mat'],...
        'clusters','pval','t_sums','permutation_distribution','sig_clust','sig_time_pts');
    ...
    %Save output
    cd(save_dir)
    save(['timecourse_cluster_unaware_eye_',eye,'_',current_eye_name,'_',num2str(num_permutations),'perm_run_baseline.mat'],...
        'clusters','pval','t_sums','permutation_distribution','sig_clust','sig_time_pts');
    toc
